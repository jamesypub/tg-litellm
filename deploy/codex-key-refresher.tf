...
locals {
  ...
  refresher_enabled           = var.enable_codex_key_refresh && local.refresher_secret_will_exist
    ...
    content  = <<-PY
      ...
      def handler(event, context):
          ...
          boto3.client("ecs", region_name=region).update_service(
              cluster=os.environ["ECS_CLUSTER"], service=service, forceNewDeployment=True)

          return {"ok": True, "rolled": service}
    PY
  }
}
