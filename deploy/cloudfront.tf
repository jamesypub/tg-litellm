...
variable "cloudfront_origin_read_timeout" {
  type    = number
  default = 120 # #46/#125: this is an INTER-PACKET (idle) timeout — a no-output gap
  # longer than this closes the stream mid-response ("Connection closed mid-response").
  # 120 is the CloudFront "Response timeout per origin" quota default — applyable on
  # any account without a Service Quotas increase. Values above 120 require a granted
  # increase; without it, apply fails `InvalidOriginReadTimeout`. LiteLLM has no SSE
  # keep-alive (researched on #46), so raising the idle timeout is the supported
  # mitigation. See the streaming FAQ in docs/installer.md.
  description = "CloudFront origin response (idle/inter-packet) timeout in seconds. Default 120 (within the CloudFront quota default — no approval needed). Raise toward 180 for longer thinking pauses AFTER a CloudFront 'Response timeout per origin' Service Quotas increase."

  validation {
    condition     = var.cloudfront_origin_read_timeout >= 30 && var.cloudfront_origin_read_timeout <= 180
    error_message = "cloudfront_origin_read_timeout must be 30–180 (values above your account's CloudFront origin-timeout quota fail apply until you request a Service Quotas increase)."
  }
}
