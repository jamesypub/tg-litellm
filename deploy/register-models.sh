...
#   WIZARD_FAMILY=claude|mantle|both  (default: both) which model FAMILIES to
#                              DISCOVER and register (#115 Issue 0). claude = skip
#                              all OpenAI/Mantle+gpt-oss discovery and rows; mantle
#                              = skip Claude discovery and rows; both = every family
#                              (today's behavior). Distinct from WIZARD_DELETE_SCOPE
#                              (that is only what to DELETE in clean mode).
...
SCOPE_SET="${SCOPE+1}"
WIZARD_FAMILY_SET="${WIZARD_FAMILY+1}"
WIZARD="${WIZARD:-0}"
[ -n "${WIZARD_MODE_SET:-}" ] || WIZARD_MODE="clean"
[ -n "${WIZARD_DELETE_SCOPE_SET:-}" ] || WIZARD_DELETE_SCOPE="both"
[ -n "${SCOPE_SET:-}" ] || SCOPE="both"
# Model-FAMILY axis (#115 Issue 0) — which providers to DISCOVER/register, distinct
# from WIZARD_DELETE_SCOPE (what to delete in clean mode). Defaults to "both" so a
# non-wizard/legacy run discovers every family exactly as before; the wizard sets it
# from the family question. claude = skip all OpenAI/Mantle discovery + rows.
# Same _SET treatment as the sibling axes above: an explicit empty WIZARD_FAMILY=''
# stays empty so wizard_validate_enums catches it, rather than being silently
# promoted to "both" by ${VAR:-both} (Codex #115 HIGH-2 fail-closed parity).
[ -n "${WIZARD_FAMILY_SET:-}" ] || WIZARD_FAMILY="both"
...
wizard_prompts() {
  ...
  if [ -z "${SCOPE_SET:-}" ]; then
    # #115 Issue 1: default the bare-alias scope to "us" (idx 1), not "both".
    # "us" is safe and always available; "global" is the optimization once
    # cross-region routing is confirmed working. This branch only runs when SCOPE
    # was NOT supplied (so it's the interactive default; a supplied SCOPE=… still
    # wins via SCOPE_SET and skips the prompt entirely, non-interactive path
    # unchanged). Both remains selectable (global-preferred + us fallback).
    local scope_default_idx=1
    case "$SCOPE" in us) scope_default_idx=1 ;; global) scope_default_idx=2 ;; both) scope_default_idx=1 ;; esac
    wizard_ask SCOPE \
      "Choosing which routing scope wins the bare (unscoped) Claude alias. This never changes which global/<model> and us/<model> aliases get registered — both are always registered (#20). 'us' is the safe default; 'global' optimizes once cross-region is confirmed." \
...
wizard_validate_enums() {
  ...
  case "$WIZARD_FAMILY" in claude|mantle|both) ;; *) echo "ERROR(#115): WIZARD_FAMILY='$WIZARD_FAMILY' invalid (want claude|mantle|both)" >&2; exit 1 ;; esac
}
wizard_prompts
wizard_validate_enums
...
# Family filter (#115 Issue 0): a Mantle-only wizard run skips Claude discovery
# entirely — no list-inference-profiles calls, empty CLAUDE/BARE, and no
# "no Claude profiles" WARNING/FAILS (an empty Claude set is intended here, not a
# discovery failure). Non-wizard/both/claude runs discover Claude exactly as before.
GLOBAL_PROFILES=(); US_PROFILES=()
if [ "$WIZARD_FAMILY" = "mantle" ]; then
  echo "==> OpenAI/Mantle-only selected — skipping Claude (Bedrock inference-profile) discovery."
else
...
if [ "$WIZARD_FAMILY" = "mantle" ]; then
  : # Claude intentionally skipped (OpenAI/Mantle-only) — empty CLAUDE is expected, not a failure.
...
# Family filter (#115 Issue 0): when the operator chose "Claude only" the wizard
# sets WIZARD_FAMILY=claude (the model-FAMILY axis — distinct from
# WIZARD_DELETE_SCOPE, which is only "what to delete in clean mode"). Probing
# Mantle then is pure noise: every openai.* row comes back "not reachable, cannot
# register" and clutters a Claude-only run. Skip the probe entirely and leave
# MANTLE_OK empty (already =() below) so no OpenAI rows enter selection. Only the
# wizard's family axis gates this; a non-wizard or both/mantle run probes as before.
...
# Family filter (#115 Issue 0): a Claude-only wizard run makes NO Mantle probe
# calls — every openai.* would come back unreachable and just add noise. Leave
# MANTLE_OK empty so no OpenAI rows enter selection. Both/mantle (and non-wizard)
# runs probe as before. The candidates + EOL/legacy tables above are declared
# unconditionally (selection/classification need them regardless of family).
if [ "$WIZARD_FAMILY" = "claude" ]; then
  echo "==> Claude-only selected — skipping Mantle (Responses API) discovery (no OpenAI models to register)."
else
...
# Family filter (#115 Issue 0): gpt-oss is an OpenAI-family provider, so a
# Claude-only wizard run skips this query too (no OpenAI rows anywhere).
OSS_RAW=""; OSS=()
if [ "$WIZARD_FAMILY" = "claude" ]; then
  : # Claude-only — no gpt-oss (OpenAI family) discovery
elif OSS_RAW="$(AWS_PROFILE="$DISCOVER_PROFILE" aws bedrock list-foundation-models --region "$REGION" --by-provider openai \
...
wizard_select() {
  ...
  # Family filter (#115 Issue 0): in a Claude-only wizard run, don't list any
  # OpenAI rows at all — neither the unreachable Mantle candidates nor gpt-oss.
  # MANTLE_OK is already empty (probe skipped above); this suppresses the "show,
  # don't select" noise rows too, so the Step 4 checklist is Claude-only.
  if [ "$WIZARD_FAMILY" != "claude" ]; then
...
wizard_delete_phase() {
  ...
  # Redisplay the FINAL selected model list before confirming (#115 Issue 2), so
  # the operator confirms the exact set they toggled in Step 4 — not just a count.
  # SELECTED holds the keys the register loops will act on; sort for stable output.
  local -a sel_keys=()
  local sk
  for sk in "${!SELECTED[@]}"; do sel_keys+=("$sk"); done
  local sel_n="${#sel_keys[@]}"
  if [ "$sel_n" -gt 0 ]; then
    IFS=$'\n' sel_keys=($(printf '%s\n' "${sel_keys[@]}" | sort)); unset IFS
  fi
  ...
  echo "   selected models:   $sel_n"
  for sk in "${sel_keys[@]}"; do echo "     [x] $sk"; done
  echo "   will register:     $reg_forms name(s) across the $sel_n selected model(s)"
  ...
  if [ "$WIZARD_TTY" = "1" ]; then
    local ans; read -r -p "Proceed with these $sel_n model(s)? [y/N]: " ans
...
#
# Dry-run (#115 Issue 5): a preview registers/deletes NOTHING, so the live
# /v1/models set reflects the gateway's CURRENT contents — not what this preview
# would produce. Reporting "all N serving (complete)" there reads as the plan's
# outcome and can list models this run never touched (e.g. pre-existing Mantle or
# hand-added rows). In dry-run, show the current state explicitly labelled as
# unchanged-by-the-preview, and never fail closed on it — the "[dry-run] … =>"
# register/delete lines above are the authoritative preview.
if [ "$DRY_RUN" = "1" ]; then
  CUR_N="$(served_count)"
  echo "==> (dry-run) gateway currently serves ${CUR_N} models — this preview changed nothing (see the [dry-run] lines above for the plan):"
  tg_curl "Authorization: Bearer" "$LITELLM_MASTER_KEY" -s "$LITELLM_URL/v1/models" \
    | python3 -c "import sys,json;[print('   -',m['id']) for m in json.load(sys.stdin).get('data',[])]" 2>/dev/null || true
  SERVED_N="$CUR_N"; PERSISTED_N="$CUR_N"
elif [ "$WIZARD" = "1" ] && [ "${WIZARD_SELECTED_N:-0}" -eq 0 ]; then
