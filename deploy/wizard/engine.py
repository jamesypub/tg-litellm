...
def build_env(answers: dict) -> dict:
    ...
    # Model families → the FAMILY axis. Two engine vars derive from this one
    # answer, both validated fail-closed against {claude, mantle, both}:
    #   WIZARD_FAMILY       — which families to DISCOVER/register (#115 Issue 0):
    #                         Claude-only skips all OpenAI/Mantle discovery+rows.
    #   WIZARD_DELETE_SCOPE — which families to DELETE in clean mode. We keep it
    #                         aligned with the chosen family so a Claude-only run
    #                         never deletes Mantle rows (and vice-versa).
    # Neither is the region-scope axis (SCOPE ∈ {us, global, both}), which the
    # wizard v1 leaves at its register-models.sh default.
    fam = _FAMILY_TO_SCOPE.get(str(answers.get("families", "")), "both")
    env["WIZARD_FAMILY"] = fam
    env["WIZARD_DELETE_SCOPE"] = fam
    ...
    for key, var in (
        ...
        # add-account: which env var holds the bearer key for this account.
        ("bedrock_api_key_env", "BEDROCK_API_KEY_ENV"),
    ):
        if answers.get(key):
...
def run(answers: dict, *, dry_run: bool, confirm: bool) -> int:
    ...
    # When a named profile is set, static credential env vars must be absent —
    # otherwise boto3/AWS CLI ignores the profile and uses the inherited creds.
