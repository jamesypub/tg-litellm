...
def done(served_hint: str = "") -> None:
    ...
    print(flush=True)
    print("  ✓ Done — your gateway's model catalog is up to date.", flush=True)
    if served_hint:
        print(f"    {served_hint}", flush=True)
    print("    Next: sign in to the admin UI and mint a developer key", flush=True)
    print("          (see docs/admin.md).", flush=True)
