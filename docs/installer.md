...
One ECS-hosted LiteLLM gateway in your AWS account, in front of Bedrock:
...
This install guide sets up the **primary account** (Claude + optional Codex on the
same account). After install, you can add additional AWS accounts for Mantle/Codex
models with `tgw add-account` — no Terraform needed for those. See
[admin.md → Adding a second AWS account](admin.md#adding-a-second-aws-account).
...
| Pricing | LiteLLM built-in cost map (auto) | confirm rates match your billing before production |
...
| ECS egress | unrestricted outbound | VPC Endpoints + restricted security group (see [security-hardening.md](security-hardening.md)) |
| Image pin | tag only (e.g. `v1.92.0`) | tag + SHA256 digest (see [security-hardening.md](security-hardening.md)) |
...
- Python package `boto3` (`pip3 install boto3`) — used by the model registration
  and add-account wizard scripts.
...
GATEWAY_URL="${CF_URL:-$(terraform output -raw alb_url)}"
...
and reachable Codex models, and registers them — using LiteLLM's built-in cost map
where available. Confirm the displayed rates match your billing before going live. It reports
...
**Resetting a gateway's model set:** the script is additive by default. Add
`WIZARD=1` to walk through a short interactive wizard (mode, what to delete,
which scope wins the bare Claude alias) with one summary before anything
happens, or `WIZARD=1 WIZARD_CONFIRM=1` to reset non-interactively. Two vars
worth knowing inline: `WIZARD_DELETE_SCOPE=claude|mantle|both` (what to delete)
and `SCOPE=us|global|both` (which scope wins the bare alias). Full var
reference is in the script header and [admin.md](admin.md#models).
...
The admin creates per-user virtual keys with budgets (LiteLLM admin UI → Virtual
Keys → Create), then points each developer at
[example/developer-setup/](../example/developer-setup) for client setup. See
[admin.md](admin.md).
...
**120 seconds** — the CloudFront "Response timeout per origin" quota default,
applyable on any AWS account without a quota change. `deploy.sh` reconciles the
ALB idle timeout to match. 120s covers most agent turns; single silent pauses
longer than 120s still cut the stream.

**To tolerate longer pauses** (very long thinking turns): raise the value.

```bash
# in $ENV.tfvars — requires a granted Service Quotas increase first
...
--quota-code L-AECE9FA7 --desired-value <N>`).
...
**Or use your own domain + certificate (enterprise).** Terminate HTTPS on your
company domain by setting a custom ACM cert on both hops: `acm_certificate_arn` +
`allow_plaintext_alb = false` (ALB), and `cloudfront_acm_certificate_arn` +
`cloudfront_aliases` (CloudFront). The gateway is then reachable at your own HTTPS
domain, which corporate proxies treat as a normal trusted site — and (unlike the
default `*.cloudfront.net`) a custom cert also enforces the TLS 1.2+ viewer
minimum (see "What TLS minimum does the edge enforce?" above). This is the common
enterprise posture where a proxy is stricter about non-corporate/uncategorized
domains.
