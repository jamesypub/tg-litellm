...
def main():
    ...
    # the fresh key is served only after the rolled task reaches steady state. Wait
    # for it, and print success ONLY then; fail on timeout. Printing before the roll
    # is stable leaves the old task (old key) serving traffic -> 401 'token expired'.
    try:
    ...
    print(f"rotated Codex key in {secret}; rolled {service} (stable)")
