...
OpenAI/Anthropic-compatible endpoint in front of Amazon Bedrock — serving both
Claude models and Codex/OpenAI (Mantle) models — with per-user virtual keys,
budgets, and hard spend caps. Built on the official open-source
...
Developer laptops
  ...
  Codex CLI    ─────────────────────────────┤
                              ...
                              └────────┬────────┬────────┘
                                       │        │
                          primary AWS account   secondary AWS account
                          Claude (Bedrock)      Mantle/Codex (Bedrock)
                          claude-opus-4-8       acme/openai.gpt-5.6-sol
                          openai.gpt-5.6-sol    (namespace-prefixed)
```

Primary-account models are registered without a prefix (e.g. `claude-opus-4-8`).
Secondary accounts use a **namespace prefix** you choose (e.g. `acme/openai.gpt-5.6-sol`)
so there are no name collisions. A developer's virtual key is scoped to exactly the
models they're allowed to use — one gateway, multiple accounts, full spend
attribution per developer.
...
**Multi-account:** `tgw add-account` onboards a second AWS account (Codex/Mantle
models) to an existing gateway — no Terraform required. Each secondary account gets
its own namespace prefix (e.g. `acme/openai.gpt-5.6-sol`), dedicated team, and
virtual key. See [admin.md → Adding a second AWS account](docs/admin.md#adding-a-second-aws-account).
...
## 4. For team managers
No separate guide needed — the LiteLLM admin UI is intuitive. Sign in and view
your team's spend, budgets, and per-developer usage under **Usage** / **Teams**.
To request a change (raise a budget, add a user or model), use your organization's
usual request process (email, internal ticket — whatever your team uses today);
an admin actions it.

## Security hardening  →  [docs/security-hardening.md](docs/security-hardening.md)

ECS egress restriction (VPC Endpoints + security group), image digest pinning, version
upgrade procedure, and secret rotation schedule. Apply before going live with real keys.
...
edge TLS + origin lock — CloudFront terminates HTTPS at the edge and a WAF +
security group let only CloudFront reach the ALB. Worth doing before real keys
and spend flow through it.
