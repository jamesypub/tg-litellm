...
| `demo0.config.toml` | `~/.codex/lg-GATEWAY-mantle-on-PREFIX.config.toml` | second-account profile template (rename with your gateway label and prefix) |
| `acme-models.json` | `~/.codex/model-catalogs/lg-GATEWAY-mantle-on-PREFIX.json` | second-account catalog sample — replace `acme/` slugs with your actual prefix; `tgw add-account` generates this automatically |
